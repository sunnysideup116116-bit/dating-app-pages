...
_flutter.loader.load({config: {canvasKitBaseUrl: new URL('canvaskit/', appBase).href}});

// Public app resources only; account content is managed by cache_bridge.js.
if ('serviceWorker' in navigator) {
  addEventListener('load', () => {
    ...
    navigator.serviceWorker.register(new URL('app-shell-sw.js', appBase), {
      scope: appBase.pathname,
      updateViaCache: 'none',
    }).then(async registration => {
      await registration.update();
      registration.waiting?.postMessage({type: 'SKIP_WAITING'});
    }).catch(error => console.info('Offline app shell unavailable:', error));
  });
}
